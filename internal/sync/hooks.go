...
func RunHooks(ctx context.Context, cfg *config.Config, phase string, sqlFile string, eventCh chan<- Event, step int) error {
	...
	for _, script := range entries {
		exposeSecrets, err := hookUsesDBSecrets(script)
		if err != nil {
			return fmt.Errorf("read hook %s: %w", filepath.Base(script), err)
		}
		sendEvent(ctx, eventCh, Event{Type: EvLog, Step: step,
			Message: fmt.Sprintf("  running hook: %s", filepath.Base(script))})
		if exposeSecrets {
			sendEvent(ctx, eventCh, Event{Type: EvLog, Step: step,
				Message: "  exposing DB password variables to hook for legacy compatibility"})
		}

		cmd := exec.CommandContext(ctx, "bash", script)
		cmd.Env = hookEnv(cfg, sqlFile, exposeSecrets)
...
func hookUsesDBSecrets(script string) (bool, error) {
	data, err := os.ReadFile(script)
	if err != nil {
		return false, err
	}
	content := string(data)
	return strings.Contains(content, "src_dbpass") || strings.Contains(content, "dst_dbpass"), nil
}
...
func hookEnv(cfg *config.Config, sqlFile string, exposeSecrets bool) []string {
	base := os.Environ()
	src := cfg.Source
	dst := cfg.Destination
	...
	extra := []string{
		...
		// Leave php empty so the binary command still runs unchanged.
		"dst_path_to_php=",
	}
	if exposeSecrets {
		extra = append(extra,
